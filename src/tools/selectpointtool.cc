...
SelectPointTool::SelectPointTool()
   : m_boundingBox( NULL),
     ...
     m_x1( 0.0 ),
     m_y1( 0.0 ),
     m_selectionMode( Model::SelectPoints )
{
}
...
void SelectPointTool::mouseButtonDown( Parent * parent, int buttonState, int x, int y )
{
   if ( m_tracking )
   {
      return;
   ...
   parent->getModel()->setSelectionMode( Model::SelectPoints );
...
void SelectPointTool::mouseButtonMove( Parent * parent, int buttonState, int x, int y )
{
   if ( m_tracking )
   {
      double x1 = m_x1;
...
const char ** SelectPointTool::getPixmap()
{
   return (const char **) selectpointtool_xpm;
}
...
{
   return QT_TRANSLATE_NOOP( "Tool", "Select Points" );
}

bool SelectPointTool::getKeyBinding( int arg, int & keyBinding )
{
   keyBinding = Qt::Key_T;
   return true;
}
