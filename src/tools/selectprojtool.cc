...
SelectProjectionTool::SelectProjectionTool()
   : m_boundingBox( NULL),
     ...
     m_x1( 0.0 ),
     m_y1( 0.0 ),
     m_selectionMode( Model::SelectProjections )
{
}
...
void SelectProjectionTool::mouseButtonDown( Parent * parent, int buttonState, int x, int y )
{
   if ( m_tracking )
   {
      return;
   ...
   parent->getModel()->setSelectionMode( Model::SelectProjections );
   ...
   parent->getModel()->setDrawProjections( true );
...
void SelectProjectionTool::mouseButtonMove( Parent * parent, int buttonState, int x, int y )
{
   if ( m_tracking )
   {
      double x1 = m_x1;
...
const char ** SelectProjectionTool::getPixmap()
{
   return (const char **) selectprojtool_xpm;
}
...
{
   return QT_TRANSLATE_NOOP( "Tool", "Select Projections" );
}

bool SelectProjectionTool::getKeyBinding( int arg, int & keyBinding )
{
   keyBinding = Qt::Key_T;
   return true;
}
