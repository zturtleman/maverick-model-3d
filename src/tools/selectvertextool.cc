...
SelectVertexTool::SelectVertexTool()
   : m_boundingBox( NULL ),
     ...
     m_x1( 0.0 ),
     m_y1( 0.0 ),
     m_selectionMode( Model::SelectVertices )
{
}
...
void SelectVertexTool::mouseButtonDown( Parent * parent, int buttonState, int x, int y )
{
   if ( m_tracking )
   {
      return;
   ...
   parent->getModel()->setSelectionMode( Model::SelectVertices );
...
void SelectVertexTool::mouseButtonMove( Parent * parent, int buttonState, int x, int y )
{
   if ( m_tracking )
   {
      double x1 = m_x1;
...
const char ** SelectVertexTool::getPixmap()
{
   return (const char **) selectvertextool_xpm;
}
...
{
   return QT_TRANSLATE_NOOP( "Tool", "Select Vertices" );
}

bool SelectVertexTool::getKeyBinding( int arg, int & keyBinding )
{
   keyBinding = Qt::Key_V;
   return true;
}
